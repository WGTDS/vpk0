 ...
 ***************************************************************************/
 /*--------------------------------------------------------------------------
    HALKEN vpk0 Tool ( Indefinitely Incomplete, but Functional ) v0.75.4

    Author  : White Guy That Don't Smile
    Date    : 2022/09/26, Monday, September 26th; 1812 HOURS
...
#ifndef USE_UTILITY_MACROS
#define USE_UTILITY_MACROS
#endif



#ifdef USE_UTILITY_MACROS
/*---------------------------------------------------------------------*
 *  at face value, this appears to be inane.                           *
 *  the way this compiles, however, yields favorable results for -O3.  *
 *---------------------------------------------------------------------*/
#define _endian( data, size ) \
{ \
  size_t n = size; \
  size_t q = size / 2; \
  char *lo = (char *)data; \
  char *hi = (char *)((size_t)data + (size - 1)); \
  char t; \
\
  do \
  { \
...
#else
static void _endian( void *data, size_t n )
{
  size_t q = n / 2;
  char *lo = (char *)data;
  char *hi = (char *)((size_t)data + (n - 1));
  char t;
...
#else
static void _verror( int status )
{
  printf( "/// ERROR ///\n>>> Vector " );
  printf( "%s", _vsubject[status] );
  printf( " is NULL!\n" );
...
#else
static void vgetval( vec_t *vec, void *data )
{
  if ( vec == (vec_t *)0 )
  {
    _verror( NULL_VEC_INFO );
...
static vec_t *_lz( const u8 *src, const u32 srclen )
{
  size_t cp;
  u32    match_offset;
  u32    match_length;
...
static vec_t *encode( const u8 *src, const u32 srclen, const u32 sample_depth )
{
  i32 flags  = 0;
  u32 nflags = 0;
  u32 breadth;
  ...
  vec_t *lz = (vec_t *)0;
#ifdef USE_UTILITY_MACROS
  vec_t *bit_stream = (vec_t *)0;

  valloc( bit_stream, sizeof(u16) );
#else
  ...
  _store_bits( bit_stream, (u16)(srclen >> 16), 16 );
  _store_bits( bit_stream, (u16)(srclen      ), 16 );
  ...
  while ( lz->cur < (void *)((size_t)lz->org + (lz->ct * lz->wd)) )
  {
    if ( nflags == 0 )
    {
      vgetval( lz, &flags );
    ...
    else
    {
      ...
      if ( bit )
      {
        ...
        if ( !sample_depth )
        {
          ...
          _store_bits( bit_stream, hword, breadth );
        }
        else
        {
          div = ((hword + 8) / 4);
        ...
        _store_bits( bit_stream, (u16)byte, breadth );
      }
      else
      {
        vgetval( lz, &byte );
        _store_bits( bit_stream, (u16)byte, 8 );
      }
...
----------------------------------*/
static vec_t *decode( const u8 *src, const u32 dstlen )
{
  i32    nbits;
  u32    codes;
  u16   *stream;
  ...
  valloc( dst, sizeof(u8) );
#else
  vec_t *dst = valloc( sizeof(u8) );
#endif

  /* vector info override */
  dst->sz  = dstlen;
  dst->org = realloc( dst->org, dst->sz );
  dst->cur = dst->org;
  dst->nxt = (void *)((size_t)dst->org + dst->sz);
  /* set dst_p */
  dst_p    = dst->org;
...
#define _reconstruct_value \
  ...
  nbits = 8;
  length = *(u32 *)(src + 4);
  _endian( &length, sizeof(u32) );
  length <<= 0x10;
  codes = *(u16 *)(src + 8);
  _endian( &codes, sizeof(u16) );
...
static int _display_error( const u32 errcode, const void *data )
{
  switch ( errcode )
  {
    ...
    case BAD_FILE_MAGIC:
      ...
      break;
    default:
      printf( "\n##  HALKEN vpk0 Tool v0.75.4 [2022/09/26]  ##\n"
...
int main( int argc, char *argv[] )
{
  ...
  if ( character > 'D' )
  {
    dst = encode( src, isize, sample_depth );
    osize = dst->ct << 1;
    fwrite( dst->org, sizeof(u8), osize, ofile );
  }
  else
  {
    magic = *(u32 *)src;
    _endian( &magic, sizeof(u32) );
    ...
    osize = *(u32 *)(src + 4);
    _endian( &osize, sizeof(u32) );
    dst = decode( src, osize );
    fwrite( dst->org, sizeof(u8), osize, ofile );
  }
